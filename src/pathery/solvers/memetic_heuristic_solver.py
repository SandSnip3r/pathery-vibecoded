...
class MemeticHeuristicSolver(BaseSolver):
    """
    A solver that uses a genetic algorithm and a DQN-powered heuristic to trigger a local search.
    """
    ...
    def __init__(
        ...
        q_value_threshold: float = 0.5,
        local_search_generations: int = 10,
        ...
        **kwargs,
        ...
        self.local_search_generations = local_search_generations
        self.local_search_iterations = local_search_iterations

        # Initialize the DQN agent
        self.agent = DQNAgent(model_path=model_path, training=False, **kwargs)
    ...
    def _after_new_population_hook(self, generation: int):
        """
        After a new population is created, this hook checks if it's time to
        trigger a local search on the best individual.
        """
        if generation % self.local_search_generations != 0:
            return

        # Get the best individual from the population
        fitness_scores = [self._calculate_fitness(env) for env in self.population]
        best_index = np.argmax(fitness_scores)
        best_individual = self.population[best_index]

        # Use the DQN to decide whether to refine
        removal_scores, placement_scores, action_type_scores = self.agent.get_q_values(
            best_individual.grid
        )
        max_q_value = np.max(
            [
                np.max(removal_scores),
                np.max(placement_scores),
                np.max(action_type_scores),
            ]
        )
        if max_q_value > self.q_value_threshold:
            print(f"Generation {generation}: Triggering local search...")
            refined_individual = self._hill_climbing(best_individual)
            self.population[best_index] = refined_individual

    def _hill_climbing(self, env: PatheryEnv) -> PatheryEnv:
        """
        Performs a hill-climbing local search on a given individual.
        """
        current_best = env.copy()
        ...
        for _ in range(self.local_search_iterations):
            ...
            wall_positions = np.where(neighbor.grid == CellType.WALL.value)
            wall_positions = list(zip(wall_positions[1], wall_positions[0]))
            empty_squares = np.where(neighbor.grid == CellType.OPEN.value)
            empty_squares = list(zip(empty_squares[1], empty_squares[0]))
            ...
            BaseSolver._remove_wall(neighbor, wall_to_move[0], wall_to_move[1])
            BaseSolver._add_wall(neighbor, new_position[0], new_position[1])
